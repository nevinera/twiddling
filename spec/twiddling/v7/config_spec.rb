describe Twiddling::V7::Config do
  ...
  describe "settings fixtures" do
    it "parses idle_time from idle-time-8m.cfg" do
      config = described_class.from_file(fixture_path("v7", "idle-time-8m.cfg"))
      expect(config.idle_time).to eq(480)
    end

    it "parses key_repeat from key-repeat-1020.cfg" do
      config = described_class.from_file(fixture_path("v7", "key-repeat-1020.cfg"))
      expect(config.key_repeat).to eq(102)
    end

    it "parses key-repeat-disabled flags" do
      config = described_class.from_file(fixture_path("v7", "key-repeat-disabled.cfg"))
      expect(config.flags_1 & 0x01).to eq(0)
    end

    it "parses haptic-feedback-off flags" do
      config = described_class.from_file(fixture_path("v7", "haptic-feedback-off.cfg"))
      expect(config.flags_1 & 0x08).to eq(0)
    end

    it "parses button-mode-keyboard flags" do
      config = described_class.from_file(fixture_path("v7", "button-mode-keyboard.cfg"))
      expect(config.flags_1 & 0x02).to eq(0x02)
    end

    it "parses nav-sensitivity-lowered flags" do
      config = described_class.from_file(fixture_path("v7", "nav-sensitivity-lowered.cfg"))
      expect(config.flags_2).to eq(0x00)
    end

    it "parses nav-invert-x-axis flags" do
      config = described_class.from_file(fixture_path("v7", "nav-invert-x-axis.cfg"))
      expect(config.flags_2 & 0x04).to eq(0x04)
    end

    it "parses nav-up-east flags" do
      config = described_class.from_file(fixture_path("v7", "nav-up-east.cfg"))
      expect(config.flags_2 & 0x03).to eq(0x01)
    end

    it "parses no-right-mouse-button dedicated buttons" do
      config = described_class.from_file(fixture_path("v7", "no-right-mouse-button.cfg"))
      expect(config.dedicated_buttons[0]).to eq(0x00)
    end

    it "parses no-t0-dedicated dedicated buttons" do
      config = described_class.from_file(fixture_path("v7", "no-t0-dedicated.cfg"))
  ...
  describe "chord fixtures" do
    it "parses a keyboard chord" do
      config = described_class.from_file(fixture_path("v7", "single-unmodified-key.cfg"))
      kbd = config.chords.find { |c| c.type_name == :keyboard }
      expect(kbd.key_name).to eq("c")
    end

    it "parses a shifted chord" do
      config = described_class.from_file(fixture_path("v7", "shifted-key.cfg"))
      kbd = config.chords.find { |c| c.type_name == :keyboard }
      expect(kbd.modifier_names).to eq(["Shift"])
    end

    it "parses a modifier chord" do
      config = described_class.from_file(fixture_path("v7", "modifier-key.cfg"))
      kbd = config.chords.find { |c| c.type_name == :keyboard }
  ...
  describe "binary round-trip" do
    Dir[fixture_path("v7", "*.cfg")].sort.each do |path|
      ...
      it "round-trips #{name}" do
        ...
        config = described_class.from_binary(original)
        expect(config.to_binary).to eq(original)
      end
    end
  end
end
